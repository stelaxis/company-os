...
// `test` scripts are gone -- `vp run -F gatekeeper-book <task>` is what replaces them.
...
export default {
  run: {
    tasks: {
      // The app bundle first: `build` type-checks and `book.ts` imports the bytes this writes.
      'build:app': {
        command: 'vite build -c vite.app.config.ts',
        input: [{ auto: true }, ownDist],
        output: ['dist-app/**', 'src/generated/**'],
      },
      build: {
        command: 'tsc',
        input: [{ auto: true }, ownDist],
